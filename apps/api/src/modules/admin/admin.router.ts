...
adminRouter.get('/audit-logs', validate(listAuditQuerySchema, 'query'), async (req, res) => {
  ...
  const [rows, total] = await Promise.all([
    prisma.auditLog.findMany({
      ...
      take: q.take,
      select: { id: true, action: true, entity: true, entityId: true, ip: true, createdAt: true, actor: { select: { id: true, username: true } } },
    }),
...
adminRouter.get('/stats', async (_req, res) => {
  const [users, activeUsers, organizations, tickets, openTickets, messages, attachments] = await Promise.all([
  ...
  res.json({ users, activeUsers, organizations, tickets, openTickets, messages, attachments });
});
