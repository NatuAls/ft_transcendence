...
gdprRouter.post('/export', ...authed, async (req, res) => {
  res.status(201).json(await gdpr.request(req.actor!.id, 'EXPORT', originOf(req)));
});

gdprRouter.post('/export/confirm', ...authed, validate(gdprConfirmSchema), async (req, res) => {
  res.json(await gdpr.confirm(req.actor!.id, 'EXPORT', req.body.token));
});

gdprRouter.get('/export/:id/download', ...authed, async (req, res) => {
  const { path, filename } = await gdpr.downloadPath(req.actor!.id, param(req.params.id));
  if (!existsSync(path)) throw Errors.resourceNotFound('export');
  res.download(path, filename);
});

gdprRouter.post('/delete', ...authed, async (req, res) => {
  res.status(201).json(await gdpr.request(req.actor!.id, 'DELETE', originOf(req)));
});

gdprRouter.post('/delete/confirm', ...authed, validate(gdprConfirmSchema), async (req, res) => {
  res.json(await gdpr.confirm(req.actor!.id, 'DELETE', req.body.token, req.body.confirmUsername));
});
