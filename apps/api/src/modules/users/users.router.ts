...
usersRouter.get('/search', ...authed, async (req, res) => {
  res.json(await users.autocomplete(typeof req.query.q === 'string' ? req.query.q : ''));
});
...
usersRouter.get(
  ...
  async (req, res) => {
    res.json(await users.listAll(req.query as unknown as Parameters<typeof users.listAll>[0]));
  },
);

usersRouter.patch('/me', ...authed, validate(updateProfileSchema), async (req, res) => {
  res.json(await users.updateProfile(req.actor!.id, req.body));
});

usersRouter.patch('/me/preferences', ...authed, validate(updatePreferencesSchema), async (req, res) => {
  res.json(await users.updatePreferences(req.actor!.id, req.body));
});

usersRouter.put('/me/avatar', ...authed, upload.single('file'), async (req, res) => {
...
usersRouter.get('/avatars/:key', (req, res) => {
  ...
  if (!/^[0-9a-f-]{36}\.webp$/.test(key)) throw Errors.resourceNotFound('avatar');
...
usersRouter.patch(
  ...
  async (req, res) => {
    res.json(await users.setStatus(req.actor!, param(req.params.id), req.body.isActive));
  },
);
...
usersRouter.patch(
  ...
  async (req, res) => {
    res.json(await users.setGlobalRole(req.actor!, param(req.params.id), req.body.globalRole));
  },
);

usersRouter.delete('/:id', ...authed, requireGlobalAdmin(), async (req, res) => {
