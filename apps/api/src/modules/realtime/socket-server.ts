...
import { isRevoked, touchPresence, clearPresence } from '../../database/redis.ts';
...
export function createSocketServer(httpServer: HttpServer): RealtimeServer {
  ...
  async function markOnline(userId: string): Promise<void> {
    ...
    await prisma.userProfile.update({ where: { userId }, data: { isOnline: true, lastSeenAt: new Date() } }).catch(() => undefined);
    io.to('presence').emit(DomainEvents.presenceChanged, { userId, isOnline: true });
  }

  async function markOffline(userId: string): Promise<void> {
    await clearPresence(userId);
    const lastSeenAt = new Date();
    await prisma.userProfile.update({ where: { userId }, data: { isOnline: false, lastSeenAt } }).catch(() => undefined);
    io.to('presence').emit(DomainEvents.presenceChanged, { userId, isOnline: false, lastSeenAt: lastSeenAt.toISOString() });
  }
  ...
  io.on('connection', (client: Socket) => {
    void (async () => {
      try {
        ...
        if (!account || account.deletedAt || !account.isActive) throw new Error('account disabled');
        ...
        await client.join(['user:' + user.id, 'presence', ...user.organizationIds.map((id) => `org:${id}`)]);
        await markOnline(user.id);
        client.emit('connected', { userId: user.id, rooms: user.organizationIds.length + 2 });
      } catch (error) {
    ...
    client.on('heartbeat', (_body: unknown, ack?: (res: { ok: true }) => void) => {
      void (async () => {
        const user = sockets.get(client.id);
    ...
    client.on('ticket.subscribe', (body: { ticketId?: string }, ack?: (res: { ok: boolean }) => void) => {
      void (async () => {
        const user = sockets.get(client.id);
        if (!user || !body?.ticketId) return ack?.({ ok: false });
        // Re-check access at subscribe time: room membership is an authorisation decision.
        const ticket = await prisma.ticket.findUnique({ where: { id: body.ticketId }, select: { organizationId: true } });
        if (!ticket || !user.organizationIds.includes(ticket.organizationId)) return ack?.({ ok: false });
        await client.join(`ticket:${body.ticketId}`);
        ack?.({ ok: true });
      })();
    });

    client.on('ticket.unsubscribe', (body: { ticketId?: string }, ack?: (res: { ok: true }) => void) => {
      void (async () => {
    ...
    client.on('conversation.subscribe', (body: { conversationId?: string }, ack?: (res: { ok: boolean }) => void) => {
      void (async () => {
        const user = sockets.get(client.id);
    ...
    client.on('conversation.unsubscribe', (body: { conversationId?: string }, ack?: (res: { ok: true }) => void) => {
      void (async () => {
    ...
    client.on('sync', (body: { since?: string }, ack?: (res: unknown) => void) => {
      void (async () => {
        const user = sockets.get(client.id);
        if (!user) return ack?.({ ok: false });
        const since = body?.since ? new Date(body.since) : new Date(Date.now() - 5 * 60_000);
        if (Number.isNaN(since.getTime())) return ack?.({ ok: false });

        const [tickets, unreadCount] = await Promise.all([
          prisma.ticket.findMany({
            where: { organizationId: { in: user.organizationIds }, updatedAt: { gt: since } },
            select: { id: true, reference: true, status: true, priority: true, updatedAt: true, organizationId: true },
            take: 100,
