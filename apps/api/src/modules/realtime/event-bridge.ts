...
export function attachEventBridge({ io, sockets }: RealtimeServer): void {
  const relay = <P>(event: string, route: (payload: P) => { rooms: string[]; body: unknown }) =>
    events.on<P>(event, (payload) => {
      const { rooms, body } = route(payload);
      for (const room of rooms) io.to(room).emit(event, body);
    });

  relay<{ ticket: { id: string; organizationId: string } }>(DomainEvents.ticketCreated, ({ ticket }) => ({
  ...
  relay<{ ticketId: string; organizationId: string }>(DomainEvents.ticketDeleted, (p) => ({
    rooms: [`org:${p.organizationId}`],
    body: p,
  }));
  relay<{ comment: unknown; ticket: { id: string } }>(DomainEvents.commentCreated, (p) => ({
    rooms: [`ticket:${p.ticket.id}`],
    body: p,
  }));
  for (const event of [DomainEvents.commentUpdated, DomainEvents.commentDeleted]) {
    relay<{ ticketId: string }>(event, (p) => ({ rooms: [`ticket:${p.ticketId}`], body: p }));
  }
  for (const event of [DomainEvents.attachmentCreated, DomainEvents.attachmentDeleted]) {
    relay<{ ticketId: string }>(event, (p) => ({ rooms: [`ticket:${p.ticketId}`], body: p }));
  }
  for (const event of [DomainEvents.categoryCreated, DomainEvents.categoryUpdated, DomainEvents.categoryDeleted]) {
    relay<{ organizationId: string }>(event, (p) => ({ rooms: [`org:${p.organizationId}`], body: p }));
  }
  ...
  events.on<{ member: { organizationId: string; user: { id: string } } }>(DomainEvents.memberAdded, async (payload) => {
  ...
  relay<{ member: { organizationId: string } }>(DomainEvents.memberUpdated, (p) => ({
    rooms: [`org:${p.member.organizationId}`],
    body: p,
  }));
  events.on<{ organizationId: string; userId: string }>(DomainEvents.memberRemoved, async (payload) => {
    const { organizationId, userId } = payload;
    io.to([`org:${organizationId}`, `user:${userId}`]).emit(DomainEvents.memberRemoved, payload);
    for (const [socketId, user] of sockets) {
      if (user.id !== userId) continue;
  ...
  relay<{ message: { conversationId: string } }>(DomainEvents.messageCreated, (p) => ({
  ...
  for (const event of [DomainEvents.friendshipRequested, DomainEvents.friendshipAccepted, DomainEvents.friendshipRemoved]) {
    relay<{
      targetUserId?: string;
      otherUserId?: string;
      ...
      rooms: [
        ...
        ...(p.friendship?.addresseeId ? [`user:${p.friendship.addresseeId}`] : []),
        ...(p.friendship?.requesterId ? [`user:${p.friendship.requesterId}`] : []),
      ],
      body: p,
    }));
  }

  events.on<{ notification: { userId: string } }>(DomainEvents.notificationCreated, async ({ notification }) => {
    const unreadCount = await prisma.notification.count({ where: { userId: notification.userId, readAt: null } });
    io.to(`user:${notification.userId}`).emit(DomainEvents.notificationCreated, { notification, unreadCount });
  });
}

/** Used by the status page to push service health live. */
export function broadcastStatus({ io }: RealtimeServer, payload: unknown): void {
