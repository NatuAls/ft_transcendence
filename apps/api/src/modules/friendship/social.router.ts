...
socialRouter.post(
  ...
  async (req, res) => {
    res
      .status(201)
      .json(await social.openConversation(req.actor!.id, req.body.userId));
  },
);
...
socialRouter.get('/conversations/:id/messages', ...authed, async (req, res) => {
  ...
  res.json(
    await social.listMessages(req.actor!.id, param(req.params.id), page, take),
  );
});
...
socialRouter.post(
  ...
  async (req, res) => {
    res
      .status(201)
      .json(
        await social.sendMessage(req.actor!.id, param(req.params.id), req.body),
      );
  },
);
