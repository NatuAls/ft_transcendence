...
function line(level: Level, scope: string, message: string, error?: unknown): void {
  ...
  if (error instanceof Error) {
    ...
    // eslint-disable-next-line no-console
    (level === 'error' || level === 'fatal' ? console.error : console.log)(prefix, message);
  }
}

export function createLogger(scope: string) {
  return {
    fatal: (message: string, error?: unknown) => line('fatal', scope, message, error),
    error: (message: string, error?: unknown) => line('error', scope, message, error),
    warn: (message: string, error?: unknown) => line('warn', scope, message, error),
